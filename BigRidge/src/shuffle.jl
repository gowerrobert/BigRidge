function boot_shuffle(prob::Prob,options::MyOptions)
    s  =options.sketchsize;                   
    name = string("shuffle-",s,"-",Int(options.AUX[2]));
    ...
    method = SketchMethod(flopsperiter,name,stepmethod,boot_shuffle,SA,SAS,Sb,ind,sigs)
    return method;
end

    
function step_shuffle_sign_flip(prob::Prob, x::Array{Float64}, options::MyOptions, method::SketchMethod )
...
function step_shuffle(prob::Prob, x::Array{Float64}, options::MyOptions, method::SketchMethod )
...
function step_shuffle_full_indexed(prob::Prob, x::Array{Float64}, options::MyOptions, method::SketchMethod )
    s = options.sketchsize;

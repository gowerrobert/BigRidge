...
Now we denote by $Z = A S^{T} (S A S^{T})^{-1} S A$, where $S$ is our Hadamard random matrix.\\
 ...
 $S = I_{C} H D$ where $C$ is a uniform random subset of $\acco{1,\dots,n}$ of size $s$, as defined in the $Randomized \,\,Newton$ section \ref{newton}, $H$ is the $Hadamard$ matrix ($H H^{T} = n I_{n}$) and $D$ is a diagonal random matrix which values are uniformly distributed in $\acco{-1,1}$ \\

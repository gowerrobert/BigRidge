...
\rob{In the statement of these corollary's and lemmas you should state the resulting convergence rate of the algorithm. The quantity $\lambda_{\min}(A^{-1/2}E{Z}A^{-1/2})$ is not an intuitive/interpretible quantity. }
\pr
...
Notice that $S$ can be rewritten as : $S = \dsp\sum_{j=1}^{n} \epsilon_{j}f_{\pi(j)}e_{\phi(j)}^{T} $, where $\pi$ is the function $\fonc{\acco{1,\dots,n}}{\acco{1,\dots,s}}{j}{-\lfloor -\frac{j}{m} \rfloor}.$\\
 ...
 \rob{I think it should be $\pi: j \rightarrow \lceil \frac{j}{m} \rceil$}

...
$\\\\$
Let $M$ be a $n\times n$ matrix of the form $a I_{n} + $
 where $a$ and $b$ are two real numbers. Then :\\
    $a-b$ and $a+(n-1)b$ are the eigenvalues of $M$ with $a-b$ of multiplicity at least $n-1$.
...
Let's observe that by subtracting $(a-b) I_{n}$ from $M$, we obtain : $M-(a-b)I_{n} = \left(
    ...
    b                                    \\
      ...
      &               &   &   & b
    \end{array}
    \right)$ which is a matrix of range at most $1$ (of range $0$ if $b = 0$). Therefore $a-b$ is an eigenvalue of $M$ of multiplicity at least $n-1$.\\
...
Denote by $\bold{C}_{D} = (\pare{P_{\phi_{1}}^{T} D f_{\pi}^{T},\dots,P_{\phi_{n!}}^{T} D f_{\pi}^{T} } $ which is a $ n \times n! \,n$ matrix.\\
...
$\\\\$
$\dsp \lambda_{min}(A^{-\frac12}E[Z]A^{-\frac12} )  \geq  \dsp  \frac{s}{n} \frac{\lambda_{min}(A)}{\lambda_{max}(A)}\pare{1-\sqrt{\frac{n}{s(n-1)}}}
$
\end{corollary}
}
...
&=& \small  (n-1)!\left(
    ...
    By using the \textbf{lemma} \ref{eigenvalue}, $\lambda_{1} = (n-1)!\tr(f_{\pi}^{T} f_{\pi}) -\frac{(n-1)!}{n-2} \tr\pare{D f_{\pi}^{T} f_{\pi} D (J-I_{n}) }     $ and \\$\lambda_{2} = (n-1)!\tr(f_{\pi}^{T} f_{\pi})+(n-1)!\frac{n-1}{n-2} \tr\pare{D f_{\pi}^{T} f_{\pi} D (J-I_{n}) } $ are the eigenvalues of $\bold{C}_{D} \bold{C}_{D}^{T} $.\\
    Hence, $\lambda_{min}( \bold{C}_{D} \bold{C}_{D}^{T} ) = (n-1)!\tr(f_{\pi}^{T} f_{\pi}) -\frac{(n-1)!}{n-2} \tr\pare{D f_{\pi}^{T} f_{\pi} D (J-I_{n}) }.$\\
...
\emph{A modifier à partir d'ici}\\

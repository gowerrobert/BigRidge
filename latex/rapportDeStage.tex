...
%For $p_{i} = \frac{Tr(I_{C_{i}} A I_{C_{i}}^{T})}{\norm{A^{\frac{1}{2} I_{C} }^{2}_{F}}$ for $i = 1,\dots,r$,\\

 %reference here] shows that $\rho \leq 1 - \frac{\lambda_{min}(I_{C}^{T}A I_{C})}{\norm{A^{\frac 12} I_{C} }^{2}_{F}}$.\\ [ref Rob thesis] \\

$\lambda_{min}(A^{-\frac12}E[Z]A^{-\frac12} ) \geq \lambda_{min}(\bold{C}^{T}A\bold{C} ) \lambda_{min}(D^{2})$

$\dsp \lambda_{min}(D^{2}) =  \min_{i} p_{i} \frac{1}{\lambda_{max}(I_{C_{i}} A I_{C_{i}}^{T} ) } =  \min_{i}\frac{p_{i} }{\lambda_{max}(I_{C_{i}}^{T} I_{C_{i}}) \lambda_{max}(A)}  \geq \frac{p}{\lambda_{max}(A)} $, 

...
By definition, $A^{-\frac12}E[Z]A^{-\frac12} = \dsp\sum\limits_{i} p_{i} A^{\frac 12} S_{i}^{T} (S_{i}  A  S_{i}^{T})^{-1} S_{i} A^{\frac 12}$ \\
...
=======
  \ecag{15}{blue}{19}{
>>>>>>> 7112e8d5795ce9bb82bfa52b852d1c040bfa58e5
...
$\\\\$
Let $M$ be a  matrix and $\mathfrak{S}_{n}$ be the set of all permutations of $\acco{1,\dots,n}$.\\
Then :\\
$\dsp\sum\limits_{\phi\in\mathfrak{S}_{n}} P_{\phi}^{T} M P_{\phi} = \small  (n-1)!\left(
    \begin{array}{ccccc}
    \tr(M)                                    \\
      & \tr(M)               &   & \textbf{\Large $\frac{\tr\pare{M (J-I_{n}) }}{n-1}$  }\\
      &               & \ddots               \\
      &\textbf{\Large $\frac{\tr\pare{M (J-I_{n}) }}{n-1}$  }&   & \tr(M)               \\
      &               &   &   & \tr(M)   
    \end{array}
    \right)$
...
$\\\\$
$\dsp \lambda_{min}(A^{-\frac12}E[Z]A^{-\frac12} )  \geq  \dsp  \frac{s}{n} \frac{\lambda_{min}(A)}{\lambda_{max}(A)}\pare{1-\sqrt{\frac{n}{s(n-1)}}}$ and then :\\

$\dsp \rho \leq 1 -  \frac{s}{n} \frac{\lambda_{min}(A)}{\lambda_{max}(A)}\pare{1-\sqrt{\frac{n}{s(n-1)}}}$
...
We finally finish the proof thanks to the concavity of the function $\lambda_{min}$ :\\
 ...
 \dsp \lambda_{min}(A^{-\frac12}E\croc{Z}A^{-\frac12} ) &\geq& \dsp  \frac{\lambda_{min}(A) \pare{ n! - (n-2)! \tr\pare{ f_{\pi}^{T} f_{\pi}  (J-I_{n})}    }  }{ n! \,m\, \lambda_{max}(A) }\\
  ...
  \item 
  $\\$
 \cheikh{Still trying to have some results about the subsampling case, I am currently using $\phi$ as injective functions from $\acco{1,\dots,ms}$ to $\acco{1,\dots,n}$. Will write it down tomorrow normally.}
  \end{itemize}
